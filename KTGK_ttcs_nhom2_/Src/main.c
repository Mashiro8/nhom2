...
#define LEDGREEN2_GPIO_PIN						GPIO_Pin_0
#define LEDGREEN2_PIN0						0
...
static void LEDGREEN2Control_SetStatus(GPIO_TypeDef *GPIOx, uint16_t GPIO_PIN,
		uint8_t Status) {
	// SET bit in BSRR Registers
...
static uint8_t Button3Read_Status(GPIO_TypeDef *GPIOx, uint16_t GPIO_PIN) {
	uint8_t Read_Pin = 0x00;
...
static uint8_t Button5Read_Status(GPIO_TypeDef *GPIOx, uint16_t GPIO_PIN) {
	uint8_t Read_Pin = 0x00;
...
int main(void)
{
	...
	if (Button3Read_Status(BUTTON3_GPIO_PORT, BUTTON3_GPIO_PIN) == 0) {
					delay();
					stat++;
				}
	if (stat%2 == 0) {
						delay();
						LEDGREEN2Control_SetStatus(LEDGREEN2_GPIO_PORT, LEDGREEN2_GPIO_PIN, 1);
					}
	else {
		delay();
								LEDGREEN2Control_SetStatus(LEDGREEN2_GPIO_PORT, LEDGREEN2_GPIO_PIN, 0);
	}
	if (Button5Read_Status(BUTTON5_GPIO_PORT, BUTTON5_GPIO_PIN) == 0) {
						delay();
						stat2++;
					}
		if (stat2%2 == 0) {
